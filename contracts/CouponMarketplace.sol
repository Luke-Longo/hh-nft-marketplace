...
contract FantomMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _isFantomNFT(address _nftAddress) internal view returns (bool) {
        return
            ...
            IFantomNFTFactory(addressRegistry.artFactory()).exists(_nftAddress) ||
            IFantomNFTFactory(addressRegistry.privateArtFactory()).exists(_nftAddress);
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IFantomPriceFeed priceFeed = IFantomPriceFeed(addressRegistry.priceFeed());
